...
AVRDUDE_FLAGS += -F -D
AVRDUDE_FLAGS += -b $(AVRDUDE_BDRATE)
...
GENDEPFLAGS = -MMD -MP -MF _other/.dep/$(@F).d
...
# Program the device.
program:
	#$(OBJDIR)/$(TARGET).hex $(OBJDIR)/$(TARGET).eep
...
# C1111ompile: create object files from C source files.
$(OBJDIR)/%.o : %.c
	@echo
	@echo $(MSG_COMPILING) $<
	$(CC) -c $(ALL_CFLAGS) $< -o $@
...
clean_list :
	...
	#$(REMOVE) $(ASRC:%.c=$(OBJDIR)/%.o)
	#$(REMOVE) $(ASRC:%.c=$(OBJDIR)/%.lst)

	$(REMOVE) $(SRC:%.c=$(OBJDIR)/%.lst)
	$(REMOVE) $(SRC:.c=.s)
	$(REMOVE) $(SRC:.c=.d)
	$(REMOVE) $(SRC:.c=.i)
	$(REMOVEDIR) _other/.dep
...
-include $(shell mkdir _other/.dep 2>/dev/null) $(wildcard _other/.dep/*)
